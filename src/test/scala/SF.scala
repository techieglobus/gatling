...
class ServiceFramework1 extends Simulation {
   ...
   val csvfeeder = csv(fileName = "./mavenscripts/src/test/resources/data/services/data.csv").circular
   val queryfeeder = csv(fileName = "./mavenscripts/src/test/resources/data/services/query.csv").circular
   //val bodyparams = System.getProperty("BODYPARAMS","{\"key\" : \"value\"}")
   val bodyparamsfile  = "./mavenscripts/src/test/resources/data/services/bodyparams.dat"
   val bodyparams = Source.fromFile(bodyparamsfile).getLines.mkString.replaceAll("[\n\r]", "")
   val queryparamsfile = "./mavenscripts/src/test/resources/data/services/queryparams.dat"
